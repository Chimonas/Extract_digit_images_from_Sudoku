...
def find_corners_of_largest_polygon(img):
	...
	# TODO apply check to see if selected polygon is indeed a sudoku
	# can be done by counting 81 squares inside
...
def save_image(img):
	cv2.imwrite('.//images/'+str(uuid.uuid4())+'.png', img)

def cut_rect(img, rect):
	"""Cuts a rectangle from an image using the top left and bottom right points."""
	return img[int(rect[0][1]):int(rect[1][1]), int(rect[0][0]):int(rect[1][0])]

def resize(img, size, margin=0, background=0):	
	h, w = img.shape[:2]
...
def crop_digit(img, rect, size):
	# get digit box image from sudoku image
	digit = cut_rect(img, rect) 
	...
	# identify if box has any digit or content
	bbox = find_largest_feature(digit, [margin, margin], [w - margin, h - margin])
	# crop the box with content
...
def find_sudoku_rectangle(image):
...
def enlarge_image_and_highlight_features(image, size):
	#show_image(image)
...
# Execution Sequence:
# Get image
# Extract sudoku square from image
# Clean up and highlight feature of image
# Crop digits and save

original_image = cv2.imread('.\\input.png', cv2.IMREAD_GRAYSCALE)
...
crop_digits(processed_image, 28)